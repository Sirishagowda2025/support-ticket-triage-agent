...
This agent uses **Groq** (free tier, no credit card required, very fast
inference).
...
This runs the agent against the included `data/sample_tickets.json` (10
sample tickets) and writes results to `output/routing_results.json` and
...
A small Streamlit UI is included for a more visual demo — upload tickets or
use the sample set, watch them classify live, and see color-highlighted
rows for anything flagged for human review.

```bash
streamlit run app.py
```

This opens in your browser. It uses the same `classify_ticket()` function
from `agent.py` — no duplicated logic, just a UI layer on top. The CLI
(`agent.py`) remains the primary, always-working entry point per the
challenge brief ("a UI is welcome but not required").
...
[1/10] Classifying TCK-001: Site is completely down for our whole team...
        -> Technical / Bug / Critical (confidence 0.95) -> Engineering
[7/10] Classifying TCK-007: question...
        -> General Inquiry / Low (confidence 0.4) -> Support Team  <-- FLAGGED FOR REVIEW
...
Done. 10 tickets processed, 2 flagged for human review.
```

Full sample results are committed at:
...
|---|---|---|---|---|---|
| TCK-001 | Technical / Bug | Critical | 0.95 | Engineering | False |
| TCK-007 | General Inquiry | Low | 0.4 | Support Team | **True** |
